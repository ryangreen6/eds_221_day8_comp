...
p +
  theme(panel.grid.minor=element_blank(),
        ...
        panel.background = element_rect(color="orange", fill = "blue"))
```
...
Now we're doing the end of day tasks in this same file.

```{r}
agencies <- read_csv(here("data_tidy", "agencies.csv"))
launches <- read_csv(here("data_tidy", "launches.csv"))
```

```{r}
ggplot(data=launches, aes(x=mission, y=launch_year)) + 
  geom_point(aes(color=agency)) +
  theme(legend.position = "none",
        plot.background = element_rect(color="orange", fill = "blue"),
        panel.background = element_rect(color="yellow"),
        annotate("text", x= 10, y= 20, label="Gonk", color="brown", size=200)) +
  labs(x="When?", y= "What?", title="Whaa?")
...
```{r}
ghg <- read_csv(here("data_tidy", "San_Francisco_Communitywide_Greenhouse_Gas_Inventory_20240821.csv"))
```
...
wicked_pge <- ghg %>%
  subset(Calendar_Year>=2005) %>%
  subset(str_detect(Sector_Detail2, pattern="PG&E")) %>%
  select(Calendar_Year, Sector_Detail2, Emissions_mtCO2e) %>%
  filter(Emissions_mtCO2e > 0)
```

```{r}
ggplot(data=wicked_pge, aes(x=Calendar_Year, y=Emissions_mtCO2e)) + 
  geom_jitter(aes(color=Emissions_mtCO2e)) +
  scale_color_gradient(low="orange", high="red") +
  labs(y= "Carbon Emissions\n(mt)", 
       x="Year", 
       title="CO^2 Emissions Generated by PG&E") +
...
```{r}
ggplot(data=wicked_pge, aes(group=Sector_Detail2, x=Calendar_Year, y=Emissions_mtCO2e)) +
  geom_boxplot()
